...
#create a list of relative URLs leading to knowledge documents hosted on the Hugging Face Hub.
...
# The script then aggregates all the scraped content from the URLs. This is achieved with the scrape_all_content() function,
# which iteratively calls scrape_page_content() for each URL and extracts its text.
...
# To prepare the collected text for embedding into our vector database, we load the content from the file and split it into separate
# documents using the load_docs() function. To further refine the content, we split it into individual chunks through the split_docs().
